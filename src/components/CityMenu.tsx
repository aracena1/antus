...
const CityMenu = ({ onCitySelect, availableCities, selectedCity }: CityMenuProps) => {
  ...
  const getDisplayCity = () => {
    return selectedCity || "Elige ciudad";
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          <span>{getDisplayCity()}</span>
          <ChevronRight className="h-6 w-6" />
        </button>
      </DialogTrigger>
      <DialogContent className="w-full h-full max-w-full p-0 border-none bg-white">
        <DialogTitle className="sr-only">Seleccionar Ciudad</DialogTitle>
